...
def prepare_folders(root_folder):
    ...
    path_to_save = '{}'.format(root_folder)
...
def process_message(id, msg_data, target_path):
    ...
    #lDate = rDate[:-21]
    #cDate = lDate[5:]
    #fDate = datetime.strptime(cDate, '%d %b %Y')
    nDate = date.today()
    ...
    # filename format id 3 digits - from address - subject
    filename = '{}_{}_{}'.format(nDate, subject[:50], msg['from'])
    filename = filename.replace("<", "")
    filename = filename.replace(">", "")
    filename = filename.replace("/", "")  # remove all the / chars
    filename = filename.replace(" ", "-")
    filename = re.sub('[:]','', filename) # other garbage
